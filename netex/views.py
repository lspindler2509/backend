...
from netex.util.query_db import query_proteins_by_identifier

from netex import models
from netex import serializers

from netex.models import Protein, Task, ProteinDrugInteraction, \
    Drug, Tissue, ExpressionLevel, Network, ProteinDisorderAssociation, DrugDisorderIndication, Disorder
from netex.serializers import ProteinSerializer, TaskSerializer, \
...
from netex.tasks import start_task, refresh_from_redis, task_stats, task_result, task_parameters
...
def result_view(request) -> Response:
    ...
    for node in parameters['input_network']['nodes']:
        ...
        if node_name_attribute in node:
            ...
            # node is custom node from user, not mapped to netex but will be displayed with all custom attributes
            node_id = node['id']
...
def graph_export(request) -> Response:
    ...
    for node in nodes:
        # netex_id is not interesting outside of drugstone
        # try:
        #     del node['netex_id']
        # except KeyError:
        #     pass
...
def adjacent_disorders(request) -> Response:
    ...
    data = request.data
    if 'proteins' in data:
        netex_ids = data.get('proteins', [])
        pdi_dataset = 'DisGeNET'
        # find adjacent drugs by looking at drug-protein edges
        pdis_objects = ProteinDisorderAssociation.objects.filter(protein__id__in=netex_ids,
        ...
        netex_ids = data.get('drugs', [])
        drdi_dataset = 'DrugBank'
        # find adjacent drugs by looking at drug-protein edges
        drdi_objects = DrugDisorderIndication.objects.filter(drug__id__in=netex_ids,
...
def adjacent_drugs(request) -> Response:
    ...
    # find adjacent drugs by looking at drug-protein edges
    pdi_objects = ProteinDrugInteraction.objects.filter(protein__id__in=netex_ids, pdi_dataset__name=pdi_dataset)
    ...
    def get(self, request) -> Response:
        ...
        for protein in proteins:
            try:
                ...
                pt_expressions[
                    ProteinSerializer().to_representation(protein)['netex_id']] = expression_level.expression_level
            except ExpressionLevel.DoesNotExist:
                pt_expressions[ProteinSerializer().to_representation(protein)['netex_id']] = None
