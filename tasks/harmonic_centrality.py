...
def harmonic_centrality(task_hook: TaskHook):
    ...
    task_hook.set_progress(2 / 4.0, "Computing harmonic centralities.")
    # all_dists = []
    ...
    for node in seed_ids:
        ...
        inverse_dists = np.zeros_like(dists, dtype=float)

        reachable = np.isfinite(dists)

        inverse_dists[reachable] = 1.0 / (dists[reachable] + 1.0)

        all_inverse_dists.append(inverse_dists)

    scores = sum(all_inverse_dists) / len(seed_ids)
